...
class Conformer(nn.Module):
    ...
    def forward(self, inputs: Tensor, input_lengths: Tensor=None) -> Tuple[Tensor, Tensor]:
        ...
        """
        input_lengths = torch.zeros([inputs.size(0)]) # dummy
        encoder_outputs, _ = self.encoder(inputs, input_lengths)
        outputs = self.fc(encoder_outputs[:, 0, :])
        # Modification for CSE 253
        # outputs = F.sigmoid(outputs)  TODO: un-comment this when applying to original dataset
        return outputs
